...
#        self.classifier_1 = nn.Linear(config.hidden_size, len(LABELS))
#        self.classifier_2 = nn.Linear(config.hidden_size, len(ENT_LABELS))
#        self.init_weights()
#    
#    def forward(
#        self,
...
#        loss = loss_fct(logits_1.view(-1, len(LABELS)), labels_1.view(-1)) + \
...
#class DataCollatorForTwoHeadedBert(DataCollatorMixin):
    ...
    if args.cmd == 'preprocess':
        ...
        for d in docs:
            for i, tok in enumerate(d['tokens']):
                ...
                #      LABELS[d['quote-tags'][i]], ENT_LABELS[d['entity-tags'][i]])
                print(d['doc_id'], d['par_id'], d['s_ids'][i], d['w_ids'][i], tok,
                      LABELS[d['merged-tags'][i]])
            print()
        ...
        for d in tqdm.tqdm(docs):
            ...
            for i in range(results.size()[0]):
                ...
                #print(d['tokens'][cur_word_id], LABELS[results[i]])
                results_words.append(LABELS[results[i]])
